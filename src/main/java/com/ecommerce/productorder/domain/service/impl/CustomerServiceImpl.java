...
public class CustomerServiceImpl implements CustomerService {
    ...
    @Override
    @Transactional(readOnly = true)
    public Page<CustomerResponse> searchCustomers(String name, String email, Customer.CustomerRole role, 
                                                Boolean isActive, Pageable pageable) {
        log.debug("Searching customers with criteria - name: {}, email: {}, role: {}, isActive: {}", 
                 name, email, role, isActive);

        return customerRepository.searchCustomers(name, email, role, isActive, pageable)
                .map(customerMapper::toResponse);
    }
