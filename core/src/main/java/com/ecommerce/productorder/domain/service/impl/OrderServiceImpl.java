...
public class OrderServiceImpl implements OrderService {
  ...
  public Page<OrderResponse> getOrdersByStatus(String status, Pageable pageable) {
    ...
    try {
      ...
      return orderRepository.findByStatus(orderStatus, pageable).map(orderMapper::toResponse);
  ...
  @Override
  @Transactional(readOnly = true)
  public Page<OrderResponse> getOrdersByAmountRange(
      String minAmount, String maxAmount, Pageable pageable) {
    ...
    try {
      ...
      return orderRepository
          .findByTotalAmountBetween(min, max, pageable)
          .map(orderMapper::toResponse);
  ...
  public List<OrderResponse> getOrdersNeedingAttention() {
    ...
    // As this project is for my personal practice, returning orders that are pending for more than
    // 24 hours
    LocalDateTime cutoffTime = LocalDateTime.now().minusHours(Constants.HOURS_FOR_ATTENTION_CHECK);

    return orderRepository
        .findByStatusAndCreatedAtBefore(Order.OrderStatus.PENDING, cutoffTime)
        .stream()
        .map(orderMapper::toResponse)
  ...
  private void validateAndReserveProductsAtomic(CreateOrderRequest request) {
    for (var orderItem : request.getOrderItems()) {
      ...
      int rowsUpdated =
          productRepository.reduceStockAtomic(orderItem.getProductId(), orderItem.getQuantity());
      ...
      log.info(
          "Atomically reduced stock for product ID: {} by {}",
  ...
  private void validateAndReserveProducts(CreateOrderRequest request) {
    for (var orderItem : request.getOrderItems()) {
      ...
      Product product =
          productRepository
              .findByIdWithLock(orderItem.getProductId())
              .orElseThrow(
                  () ->
                      new ResourceNotFoundException(
                          "Product not found with ID: " + orderItem.getProductId()));
  ...
  private void restoreProductStock(Order order) {
    ...
    for (OrderItem orderItem : order.getOrderItems()) {
      try {
        ...
        log.info(
            "Restored {} units of product {} (ID: {})",
            orderItem.getQuantity(),
            orderItem.getProduct().getName(),
            orderItem.getProduct().getId());
      } catch (Exception e) {
        log.error(
            "Failed to restore stock for product ID: {} in order ID: {}",
            orderItem.getProduct().getId(),
            order.getId(),
            e);
