...
public class ProductServiceImpl implements ProductService {
  ...
  public Optional<ProductResponse> getProductBySku(String sku) {
    ...
    return productRepository.findBySkuAndActiveTrue(sku).map(productMapper::toResponse);
  }
  ...
  public Page<ProductResponse> searchProducts(
      ...
      Pageable pageable) {
    log.debug(
        "Advanced product search with criteria - name: {}, category: {}, price: {} - {}",
        name,
        categoryId,
        minPrice,
        maxPrice);
    ...
    return productRepository
        .findProductsByCriteria(name, categoryId, minPrice, maxPrice, pageable)
        .map(productMapper::toResponse);
  }
  ...
  private void publishStockUpdateEvent(Product product) {
    try {
      ...
      kafkaTemplate.send(
          kafkaTopics.getProduct().getStockUpdated(), product.getId().toString(), event);
