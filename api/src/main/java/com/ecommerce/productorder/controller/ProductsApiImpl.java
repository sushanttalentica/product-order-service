...
public class ProductsApiImpl implements ProductsApi {
  ...
  public ResponseEntity<ProductResponseApi> createProduct(
    ...
    CreateProductRequest dtoRequest = new CreateProductRequest();
  ...
  public ResponseEntity<Object> getAllProducts(Integer page, Integer size) {
    Page<ProductResponse> productsPage = productService.getAllProducts(PageRequest.of(page, size));
  ...
  public ResponseEntity<ProductResponseApi> updateProduct(
    ...
    UpdateProductRequest dtoRequest = new UpdateProductRequest();
  ...
  public ResponseEntity<MessageResponse> deleteProduct(Long productId) {
    ...
    return ResponseEntity.ok(
        new MessageResponse().message("Product deleted successfully").success(true));
  }
  ...
  public ResponseEntity<List<ProductResponseApi>> searchProducts(String keyword) {
    ...
    return ResponseEntity.ok(
        products.stream().map(this::convertToApiModel).collect(Collectors.toList()));
  }
  ...
  @Override
  public ResponseEntity<List<ProductResponseApi>> getProductsByPriceRange(
      BigDecimal minPrice, BigDecimal maxPrice) {
    var products =
        productService.getProductsByPriceRange(minPrice, maxPrice, PageRequest.of(0, 100));
    return ResponseEntity.ok(
        products.stream().map(this::convertToApiModel).collect(Collectors.toList()));
  }
  ...
  public ResponseEntity<List<ProductResponseApi>> advancedSearchProducts(
    ...
    var products =
        productService.searchProducts(name, categoryId, minPrice, maxPrice, PageRequest.of(0, 100));
    return ResponseEntity.ok(
        products.stream().map(this::convertToApiModel).collect(Collectors.toList()));
  }
  ...
  @Override
  public ResponseEntity<ProductResponseApi> updateProductStock(
      Long productId, UpdateStockRequest updateStockRequest) {
    log.info("Updating stock for product: {}", productId);
    var response =
        productService.updateProductStock(productId, updateStockRequest.getStockQuantity());
    return ResponseEntity.ok(convertToApiModel(response));
  }
