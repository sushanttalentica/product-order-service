...
public class OrdersApiImpl implements OrdersApi {
  ...
  public ResponseEntity<OrderResponseApi> createOrder(CreateOrderRequestApi createOrderRequest) {
    ...
    CreateOrderRequest dtoRequest = new CreateOrderRequest();
  ...
  public ResponseEntity<Object> getAllOrders(Integer page, Integer size) {
    Page<OrderResponse> ordersPage = orderService.getAllOrders(PageRequest.of(page, size));
  ...
  public ResponseEntity<MessageResponse> cancelOrder(Long orderId) {
    ...
    return ResponseEntity.ok(
        new MessageResponse().message("Order cancelled successfully").success(true));
  }
  ...
  public ResponseEntity<List<OrderResponseApi>> getOrdersByCustomer(Long customerId) {
    ...
    return ResponseEntity.ok(
        orders.stream().map(this::convertToApiModel).collect(Collectors.toList()));
  }
  ...
  public ResponseEntity<List<OrderResponseApi>> getOrdersByDateRange(
    ...
    var orders =
        orderService.getOrdersByDateRange(
            startDate.toString(), endDate.toString(), PageRequest.of(0, 100));
    return ResponseEntity.ok(
        orders.stream().map(this::convertToApiModel).collect(Collectors.toList()));
  }
  ...
  public ResponseEntity<List<OrderResponseApi>> getOrdersByAmountRange(
    ...
    var orders =
        orderService.getOrdersByAmountRange(
            minAmount.toString(), maxAmount.toString(), PageRequest.of(0, 100));
    return ResponseEntity.ok(
        orders.stream().map(this::convertToApiModel).collect(Collectors.toList()));
  }
  ...
  public ResponseEntity<OrderStatistics> getOrderStatistics() {
    var stats = (Map<String, Object>) orderService.getOrderStatistics(null, null);
    var apiStats = new OrderStatistics();
    apiStats.setTotalOrders(
        stats.get("totalOrders") != null ? ((Number) stats.get("totalOrders")).longValue() : 0L);
    apiStats.setTotalRevenue(
        stats.get("totalRevenue") != null
            ? (BigDecimal) stats.get("totalRevenue")
            : BigDecimal.ZERO);
    apiStats.setAverageOrderValue(
        stats.get("averageOrderValue") != null
            ? (BigDecimal) stats.get("averageOrderValue")
            : BigDecimal.ZERO);
  ...
  private OrderResponseApi convertToApiModel(OrderResponse dto) {
