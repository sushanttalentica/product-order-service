...
public class PaymentServiceImpl implements PaymentService {
  ...
  public Optional<PaymentResponse> getPaymentById(String paymentId) {
    ...
    return paymentRepository.findByPaymentId(paymentId).map(PaymentResponse::fromEntity);
  }
  ...
  public Optional<PaymentResponse> getPaymentByOrderId(Long orderId) {
    ...
    return paymentRepository.findByOrderId(orderId).map(PaymentResponse::fromEntity);
  }
  ...
  public PaymentResponse refundPayment(String paymentId, String refundAmount) {
    ...
    BigDecimal refundAmountDecimal =
        (refundAmount != null && !refundAmount.trim().isEmpty())
    ...
    try {
      ...
      PaymentResponse gatewayResponse =
          paymentGatewayService.processRefund(payment, refundAmountDecimal);
  ...
  public List<PaymentResponse> getPaymentsNeedingProcessing() {
    ...
    return paymentRepository.findPaymentsNeedingProcessing().stream()
        .map(PaymentResponse::fromEntity)
        .collect(Collectors.toList());
  }
  ...
  private PaymentResponse processPaymentThroughGateway(
      Payment payment, ProcessPaymentRequest request) {
          ...
          .ifPresent(
              order -> {
                if (paymentSuccessful) {
                  ...
                  log.info(
                      "Processing payment success for order {} with current status: {}",
                      orderId,
                      currentStatus);
                  ...
                  if (currentStatus == OrderStatus.PENDING) {
                    ...
                    log.info(
                        "Order {} status updated: PENDING → CONFIRMED after successful payment",
                        orderId);
                  } else if (currentStatus == OrderStatus.CONFIRMED) {
                    order.updateStatus(OrderStatus.PROCESSING);
                    log.info(
                        "Order {} status updated: CONFIRMED → PROCESSING after successful payment",
                        orderId);
                  } else {
                    log.warn(
                        "Order {} is in {} status, no automatic update after payment",
                        orderId,
                        currentStatus);
                  }
                  ...
                  log.warn(
                      "Payment failed for order {}, order remains in {} status",
                      orderId,
                      order.getStatus());
                }
              });
